...
class TestMultiDomain(object):
    ...
    @pytest.mark.tier2
    def test_0012_filesproxy(self, multihost, multidomain_sssd, localusers):
        """
        :Title: IDM-SSSD-TC: ldap_provider: test_for_multidomain:  Attempt to
        add local user in ldap group with domain files and proxy

        multidomain_sssd(domains='files_proxy')
        """
        # 'usermod' just didn't return an error before,
        # but it never could add a local user to an LDAP group
        # https://bugzilla.redhat.com/show_bug.cgi?id=1727236

        multidomain_sssd(domains='files_proxy')
        multihost.client[0].service_sssd('start')
        usermod = 'usermod -a -G pgroup0 user5000'
        cmd = multihost.client[0].run_command(usermod, raiseonerr=False)
        assert cmd.returncode != 0

    @pytest.mark.tier2
    def test_0013_filesldap(self, multihost, multidomain_sssd):
        """
    ...
    @pytest.mark.tier2
    def test_0018_filesldap(self, multihost, multidomain_sssd, localusers):
        """
        @Title: IDM-SSSD-TC: ldap_provider: test_for_multidomain: Attempt to
    ...
    @pytest.mark.tier2
    def test_0019_filesldap(self, multihost, multidomain_sssd, localusers):
        """
        @Title: IDM-SSSD-TC: ldap_provider: test_for_multidomain: Attempt to
        add local user in ldap domain group
        ...
        multidomain_sssd(domains='local_ldap')
        multihost.client[0].service_sssd('start')
        usermod = 'usermod -a -G qgroup0@ldap1 user5000'
        cmd = multihost.client[0].run_command(usermod, raiseonerr=False)
        assert cmd.returncode != 0

    @pytest.mark.tier2
    def test_0020_filesfiles(self, multihost, multidomain_sssd):
        """
    ...
    @pytest.mark.tier2
    def test_0024_ldapldap(self, multihost, multidomain_sssd):
        """
        @Title: IDM-SSSD-TC: ldap_provider: test_for_multidomain: User
        information not updated on login for secondary domains bz678593
    ...
    @pytest.mark.tier2
    def test_0025_ldapldap(self, multihost, multidomain_sssd):
        """
